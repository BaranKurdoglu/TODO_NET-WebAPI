...
namespace dotnetDeneme.Repository
{
    public class StockRepository : IStockRepository
    {
        ...
        public async Task<List<StockDto>> GetAllAsync(QueryObject query)
        {
            ...
            {
                stocks = stocks.Where(s => s.CompanyName.Contains(query.CompanyName)); //Contains, == ile aynı anlama geliyor.
            }


            if (!string.IsNullOrWhiteSpace(query.SortBy))
            {
                if (query.SortBy.Equals("Symbol", StringComparison.OrdinalIgnoreCase)) //Büyük - Küçük harf farkını yok sayar. 
                {
